...
1. **Parses** last night's sleep metrics — total sleep, deep/REM/core stages,
   ...
   JSON format.
2. **Fetches** today's events from my Google Calendar.
...
- **httpx** for outbound API calls (Claude, Google Calendar, ntfy).
...
- Mock or stub the external APIs (Claude, Google Calendar, ntfy) — tests should
